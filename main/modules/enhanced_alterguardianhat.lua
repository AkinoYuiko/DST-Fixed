...
local function OnUpdateDapperness(owner)
	...
	if inst then
		if inst.lunarseedsmaxed then
			...
			local single_seed_dapperness = TUNING.CRAZINESS_SMALL / 5
...
local function onequip(inst, owner, ...)
	...
	inst:ListenForEvent("sanitydelta", OnUpdateDapperness, owner)

	OnUpdateDapperness(inst)
end

local function onunequip(inst, owner, ...)
	inst.onunequip_prefns["dst-fixed"](inst, owner, ...)
	inst:RemoveEventCallback("onattackother", inst.glash_fn, owner)
	inst:RemoveEventCallback("sanitydelta", OnUpdateDapperness, owner)
end
