...
近期更新：
- 适配官方更新
- 修复显示包裹会在显示特定方式生成（比如大霜鲨）的包裹时崩溃
- 新模块：阴郁双回旋镖
]],
	[[
- Make compatible with mod: Night Stories.

Recent changes:
- Make compatible with klei's update.
- Fixed Show Bundle crashes when trying to display the content of a bundle generated by some specific things like Frostjaw
...
configuration_options = {
	AddTitle(zheng("默认开启", "Default On")),
	{
		...
		hover = zheng(
			"鼠标放在包裹上时显示里面的内容",
			"Show bundles and gifts' content on mouse hover."
		),
		options = boolean,
		default = true,
	},
...

	{
		...
		hover = zheng(
			'每个角色都会使用自己的动作失败台词而不是威尔逊的 "我做不到。"',
			'Characters use theis own actionfail strings instead of "I can\'t do that."'
		),
		options = boolean,
		default = false,
	},
	{
		...
		hover = zheng(
			"灵魂状态的玩家在猎犬袭击开始时不会被统计进去",
			"Ghost players won't be stat on hounds-attack start."
		),
		options = boolean,
		default = false,
	},
	{
		...
		hover = zheng(
			"大部分检查文本将跟随客户端语言\n测试性功能，谨慎使用",
			"Make most descriptions follow client language.\n Testing func. Use at your own risk."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"启迪之冠可放入月亮碎片代替攻击时的精神消耗\n月灵被替换为另一种特效，且可以享受玩家的倍率（不会小于1倍率）和增益加成",
			"Moon Shard into Enlightened Crown"
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"月之魔典在月圆的时候读会月黑",
			"Read Lunar Grimoire on full moon will change it to new moon"
		),
		options = boolean,
		default = false,
	},
	{
		...
		hover = zheng(
			"右键使用沙之石生成一次性临时沙塔",
			"Use Sand stone in inventory to generate a temp Lazy Deserter."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"吹箭打在无效目标上面时会掉落而不是消失",
			"No blow dart waste when hitting a dead target"
		),
		options = boolean,
		default = false,
	},
	{
		...
		hover = zheng(
			"扫把可以对物品栏内的物品使用（手杖除外）",
			"Clean Sweeper can sweep inventory items (Cane not included)"
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"姐妹骨灰盒可以放骨片或哀悼荣耀",
			"Allows bone shards and Mourning glory into Sisturn."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"大触手不再掉落任何东西（减少垃圾产生）",
			"Removes Tentacle pillar's loottable"
		),
		options = boolean,
		default = false,
	},
	...
	AddTitle(zheng("世界相关", "The World")),
	{
		...
		hover = zheng(
			"【仅】普通种子在腐烂的时候消失而不是生成腐烂食物",
			"Normal seeds disappear on perished."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"人工种植的巨树被雷劈后不会掉落草和树枝",
			"Ocean Tress won't drop cutgrass, twigs on thunder strike."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"风暴事件结束时月相盘在出生门处掉落玻璃碎片",
			"Moon Shard dropped from Moon Dials will be teleported to Celestial Portal."
		),
		options = boolean,
		default = false,
	},
	...
	},
	{
		...
		hover = zheng(
			"万圣节期间猪王不会给糖果（减少垃圾产生）",
			"Disable halloween candies on trading with Pig King."
		),
		options = boolean,
		default = false,
	},
	{
		...
		hover = zheng(
			"禁用冬季盛宴期间小玩意儿和零食的掉落，但不影响BOSS的挂饰（减少垃圾产生）",
			"Disable most loots in Winter's Feast, eipcs not included."
		),
		options = boolean,
		default = false,
	},
	AddTitle(zheng("配方相关", "The Crafting")),
	{
		...
		hover = zheng(
			"【照明栏解锁】新增彩灯配方, 合成时会随机给一种款式的彩灯",
			"Enable recipe for RANDOM winter lights"
		),
		options = boolean,
		default = false,
	},
...
for i = 1, #configuration_options do
	if
		configuration_options[i].name
		and new_modules[configuration_options[i].name]
		and configuration_options[i].label
	then
